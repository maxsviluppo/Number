    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const { data: profiles, count, error } = await (supabase as any)
                .from('profiles')
                .select('username, email, total_score, max_level', { count: 'exact' });
                            ...
                            <div className="bg-[#111] border border-[#222] rounded-2xl overflow-hidden">
                                <table className="w-full text-left">
                                    <thead className="bg-[#1a1a1a] border-b border-[#222]">
                                        <tr>
                                            <th className="px-6 py-4 font-semibold text-gray-400 text-sm">Username</th>
                                            <th className="px-6 py-4 font-semibold text-gray-400 text-sm">Punteggio</th>
                                            <th className="px-6 py-4 font-semibold text-gray-400 text-sm">Livello</th>
                                    ...
                                    <tbody className="divide-y divide-[#222]">
                                        {subscribers.map((user, idx) => (
                                            <tr key={idx} className="hover:bg-[#161616] transition-colors">
                                                <td className="px-6 py-4 font-medium text-white">{user.username || 'Anonimo'}</td>
                                                <td className="px-6 py-4 text-gray-300">{(user as any).total_score || 0}</td>
                                                <td className="px-6 py-4 text-gray-300">{(user as any).max_level || 1}</td>
                                                <td className="px-6 py-4">
                                                    <span className="px-2 py-1 bg-green-900/30 text-green-400 text-xs rounded-full border border-green-900">
                                                        Attivo
                                                    </span>
                                                </td>
                                            </tr>
