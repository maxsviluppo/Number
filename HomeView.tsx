...
import { Brain, Zap, Trophy, Play, ChevronRight, Menu, X } from 'lucide-react';
  ...
  return (
    <div className="relative min-h-screen bg-[url('/sfondo.png')] bg-cover bg-center bg-no-repeat bg-fixed text-white flex flex-col font-['Inter'] overflow-hidden">
      <video
        ...
        aria-hidden="true"
      />
      <div className="fixed inset-0 z-0 bg-slate-950/55 pointer-events-none" aria-hidden="true" />
      ...
      <nav className={`fixed top-0 w-full z-[10000] border-b border-white/5 py-4 px-6 flex justify-between items-center transition-colors duration-300 ${isMenuOpen ? 'bg-black' : 'glass-panel'}`}>
        ...
        <div className="hidden md:flex gap-8 text-sm font-semibold uppercase tracking-widest text-slate-400">
          ...
          <Link to="/play" className="hover:text-[#FF8800] transition-colors font-black text-white">{t('nav.play')}</Link>
          <Link to="/invite" className="hover:text-[#FF8800] transition-colors text-[#FF8800]">{t('nav.invite')}</Link>
          <Link to="/blog" className="hover:text-[#FF8800] transition-colors">{t('nav.blog')}</Link>
          <Link to="/about" className="hover:text-[#FF8800] transition-colors">{t('nav.about')}</Link>
        </div>
        ...
        {isMenuOpen && (
          <div className="fixed inset-0 top-[72px] bg-black z-[9999] flex flex-col p-8 gap-6 animate-screen-in md:hidden h-[calc(100vh-72px)] overflow-y-auto">
            ...
            <Link to="/invite" onClick={() => setIsMenuOpen(false)} className="text-2xl font-black font-['Orbitron'] hover:text-[#FF8800] border-b border-white/5 pb-4 text-[#FF8800]">{t('nav.invite').toUpperCase()}</Link>
      ...
      <section className="relative z-10 pt-32 pb-20 px-6 max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
        <div className="flex-1 text-center lg:text-left">
          ...
          <h1 className="text-5xl md:text-7xl font-black font-['Orbitron'] mb-8 leading-[0.9]">
            {t('home.heroTitle')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#FF8800] to-[#FFBB00]">{t('home.heroTitleHighlight')}</span>
          </h1>
      ...
      <section className="relative z-10 py-20 px-6 bg-slate-900/40 border-y border-white/5">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-12 text-center">
          <div className="flex flex-col items-center group">
            ...
            <h3 className="text-xl font-bold mb-4 font-['Orbitron']">{t('home.feature1Title')}</h3>
            <p className="text-slate-400">{t('home.feature1Desc')}</p>
          </div>
          <div className="flex flex-col items-center group">
            ...
            <h3 className="text-xl font-bold mb-4 font-['Orbitron']">{t('home.feature2Title')}</h3>
            <p className="text-slate-400">{t('home.feature2Desc')}</p>
          </div>
          <div className="flex flex-col items-center group">
            ...
            <h3 className="text-xl font-bold mb-4 font-['Orbitron']">{t('home.feature3Title')}</h3>
            <p className="text-slate-400">{t('home.feature3Desc')}</p>
          </div>
        </div>
      </section>

      <footer className="relative z-10 mt-auto py-12 px-6 border-t border-white/5 bg-slate-950/90">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-10">
          ...
          <div>
            <h4 className="font-bold mb-6 uppercase text-xs tracking-widest text-[#FF8800]">{t('home.usefulLinks')}</h4>
            <ul className="space-y-4 text-sm text-slate-400">
              <li><Link to="/play" className="hover:text-white transition-colors">{t('home.playNowLink')}</Link></li>
          ...
          <div>
            <h4 className="font-bold mb-6 uppercase text-xs tracking-widest text-[#FF8800]">{t('home.legal')}</h4>
            <ul className="space-y-4 text-sm text-slate-400">
              <li><Link to="/privacy" className="hover:text-white transition-colors">{t('home.privacyPolicy')}</Link></li>
              <li><Link to="/cookies" className="hover:text-white transition-colors">{t('home.cookiePolicy')}</Link></li>
              <li><Link to="/terms" className="hover:text-white transition-colors">{t('home.termsOfUse')}</Link></li>
            </ul>
          </div>
        </div>
